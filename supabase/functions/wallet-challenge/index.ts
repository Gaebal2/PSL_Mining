...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data, error } = await admin.from("wallet_verification_challenges").insert({ user_id: user.id, wallet_address: normalized, muxed_id: muxedId, muxed_address: muxedAddress, amount: "0.0100000", network, expires_at: expiresAt }).select().single();
